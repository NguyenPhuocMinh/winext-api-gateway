    ...
    try {
      ...
      if (isEmpty(config) || !kongEnable) {
        ...
        const keyAuth = await fetch(`http://localhost:${kongPort}/${userName}/${keyName}`, {
          ...
          .then(resKey => {
            console.log("XXXX", resKey.status);
            if (resKey.status === constants.KONG_STATUS.CREATED) {
              return resKey.json();
            } else if (resKey.status === constants.KONG_STATUS.NOT_FOUND) {
              console.log("AAAAA", resKey.json());
              // response.status(resKey.status).send({ message: resKey.json().message });
            }
            else {
