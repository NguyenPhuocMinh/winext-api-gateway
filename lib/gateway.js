        ...
        if (!isEmpty(service)) {
          const data = await fetch(`http://localhost:${kongPort}/services`, {
            ...
            .then(async (resNewService) => {
              if (resNewService.status === constants.ERROR_KONG_STATUS.CREATED) {
              ...
              } else if (resNewService.status === constants.ERROR_KONG_STATUS.DUPLICATED) {
                const existService = await fetch(`http://localhost:${kongPort}/services/${service.name}`)
                  .then(resExistService => {
                    if (resExistService.status === constants.ERROR_KONG_STATUS.SUCCESS) {
