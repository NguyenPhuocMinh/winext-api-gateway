...
function ApiGateWay(params = {}) {
  ...
  this.createService = async function (request, response, next) {
    try {
      ...
      if (isEmpty(config) || !kongEnable) {
        loggerTracer.warn('function createService not enable kong gateway');
        return next();
      } else {
        ...
        if (!isEmpty(service)) {
          const data = await fetch(`http://localhost:${kongPort}/services`, {
            ...
            if (resNewService.status === constants.KONG_STATUS.CREATED) {
              ...
              if (!isEmpty(existService)) {
                loggerTracer.data(`function createService with existService has data`, {
                  args: {
          ...
          if (!isEmpty(data)) {
            loggerTracer.data(`function createService has been end with data`, {
              args: {
  ...
  this.createRouter = async function (request, response, next) {
    try {
      ...
      if (isEmpty(config) || !kongEnable) {
        loggerTracer.warn('function createRouter not enable kong gateway');
        return next();
      } else {
        ...
        if (!isEmpty(route)) {
          const data = await fetch(`http://localhost:${kongPort}/routes`, {
            ...
            if (resNewRoute.status === constants.KONG_STATUS.CREATED) {
              ...
              if (!isEmpty(existRoute)) {
                loggerTracer.data(`function createRouter with existRoute has data`, {
                  args: {
                    status: resNewRoute.status,
          ...
          if (!isEmpty(data)) {
            loggerTracer.data(`function createRouter has been end with data`, {
              args: {
                nameRoute: data.name,
                idRoute: data.id,
              },
  ...
  this.createConsumer = async function (request, response, next) {
    try {
      ...
      if (isEmpty(config) || !kongEnable) {
        loggerTracer.warn('function createConsumer not enable kong gateway');
        return next();
      } else {
        ...
        if (!isEmpty(consumer)) {
          const data = await fetch(`http://localhost:${kongPort}/consumers`, {
            ...
            if (resNewConsumer.status === constants.KONG_STATUS.CREATED) {
              ...
              if (!isEmpty(existConsumer)) {
                loggerTracer.data(`function createConsumer with existConsumer has data`, {
                  args: {
          ...
          if (!isEmpty(data)) {
            loggerTracer.data(`function createConsumer has been end with data`, {
              args: {
  ...
  this.createPlugin = async function (request, response, next) {
    try {
      ...
      if (isEmpty(config) || !kongEnable) {
        loggerTracer.warn('function createPlugin not enable kong gateway');
        return next();
      } else {
        ...
        if (!isEmpty(plugin)) {
          const data = await fetch(`http://localhost:${kongPort}/plugins`, {
            ...
            if (
              ...
              loggerTracer.data(`function createPlugin with response status created successfully or exist plugin`, {
                args: {
          ...
          if (!isEmpty(data)) {
            loggerTracer.data(`function createPlugin has been end`);
            return next();
          }
        }
      }
  ...
  this.authGateway = async function (request, response, next) {
    try {
      ...
      if (isEmpty(config) || !kongEnable) {
        loggerTracer.warn('function authGateway not enable kong gateway');
        return next();
      } else {
        ...
        const keyAuth = await fetch(`http://localhost:${kongPort}/consumers/${userName}/${keyName}`, {
          ...
          if (resKey.status === constants.KONG_STATUS.CREATED) {
            loggerTracer.data(`function authGateway get key auth successfully!`);
            return resKey.json();
          } else {
        ...
        if (!isEmpty(keyAuth)) {
          loggerTracer.data(`function authGateway has been end with key auth`, {
            args: {
              keyAuth: keyAuth.key,
          ...
          if (isEmpty(gatewayKeyInHeaders)) {
            ...
            if (!isEmpty(findEnablePath)) {
              loggerTracer.data(`function authGateway has findEnablePath`);
              return next();
            }
            /**
             * findPathPublic
            ...
            if (!isEmpty(findPathPublic) && !enablePublicPath) {
              loggerTracer.data(`function authGateway has findPathPublic and not enablePublicPath`, {
            ...
            if (!isEmpty(findProtectedPath) && !enableProtectedPath) {
              loggerTracer.data(`function authGateway has findProtectedPath and not enableProtectedPath`, {
